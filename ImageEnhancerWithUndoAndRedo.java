...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    public ImageEnhancerWithUndoAndRedo() {
        ...
        undoStack = new BufferedImageStack();// stack for undos
        redoStack = new BufferedImageStack();// stack for redos
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() != undoItem && e.getSource() != redoItem) { // checks if action is not undo or redo, if so:
            if (biFiltered == null) { //checks if biFiltered is null (first time you run an action) if so, sets biFiltered to biTemp (starting image)
                biFiltered = biTemp;
            }
            undoStack.push(biFiltered); //	puts biFiltered into the undo stack to allow for undoing
            for (int i = 0; i < redoStack.getSize(); i++) {// deplets the redo stack of all its values
                redoStack.pop();
            }
        }

        if (undoStack.getSize() > 0) { // if size of undo stack is greater than 0, enabled the undo button
            undoItem.setEnabled(true);
            if (e.getSource() == undoItem) {// if action is undo:
                BufferedImage image = undoStack.pop();// remove top image from undo and put it into a variable
                redoStack.push(biFiltered);// push biFiltered into the redo Stack (before biFiltered gets changed to image variable)
                biFiltered = image;// sets biFiltered to image variable
            }
        }
        if (redoStack.getSize() > 0) {// if size of undo stack is greater than 0, enabled the redo button
            redoItem.setEnabled(true);
            if (e.getSource() == redoItem) {// if action is redo:
                BufferedImage image = redoStack.pop();// remove top image from redo and put it into a variable
                undoStack.push(biFiltered);// push biFiltered into the undo Stack (before biFiltered gets changed to image variable)
                biFiltered = image;// sets biFiltered to image variable
            }
        }
    ...
    public void run() {
        JFrame f = new JFrame("Image Enhancer WITH Undo AND Redo by Arnav Jain and Zachary Gaudin"); // Students should update this.
